...
async def chat_completions(
    ...
    with stage_span(
        ...
        **{"tollgate.model_requested": request.model, "tollgate.streaming": request.stream},
        ...
        # The probe opens the probe/exact/semantic span subtree itself, per
        # bench/stages.md. On a hit the stages that exist to reach a provider
        # simply do not run, which is the point of the cache.
        ...
        if lookup.outcome == "miss":
            ...
            allow_local = (
                http_request.headers.get("x-tollgate-allow-local", "").lower() == "true"
            )
            ...
            with stage_span(spans.BUDGET) as budget_span:
                ...
                # Unknown limits stay off the span rather than becoming nulls:
                # an attribute that silently drops is the honest representation
                # of "unknown", which is what None means here.
                if head.requests_per_minute.remaining is not None:
                    budget_attrs["tollgate.budget.requests_remaining"] = (
                        head.requests_per_minute.remaining
                    )
                if head.tokens_per_minute.remaining is not None:
                    budget_attrs["tollgate.budget.tokens_remaining"] = (
                        head.tokens_per_minute.remaining
                    )
            ...
            with stage_span(spans.SELECT) as select_span:
                ...
                for skip in selection.skipped:
                    await gateway.metering.append(
                        now_row("skip", skip.provider, detail=skip.reason)
                    )
        else:
            selection = None
        ...
        if selection is None:
            ...
            completion = _completion_from_entry(lookup.entry)
        else:
            ...
            try:
                with stage_span(spans.DISPATCH) as dispatch_span:
                    completion, failure = await _dispatch(
                        gateway, remaining_chain, messages, kwargs
                    )
        ...
        similarity = lookup.similarity if lookup.outcome == "semantic" else None
